...
\begin{ganttchart}[
  ...
  \ganttbar[bar/.append style={fill=orange}]{Data/Models Submission}{2025-10-01}{2025-10-01} \\
...
We hypothesize that (1) learning patterns of cross-dependencies across point mutations % in key proteins implicated in viral entry and exit (HA and NA) 
...
% The BioNORAD platform will enable proactive and actionable global surveillance for emerging pandemic threats from Influenza A, thereby protecting DoD assets and personnel deployed in potentially high-risk centers of emergence. It will also enable preemptive action, including the inoculation of animal reservoirs before the first human infection, potentially eliminating the pandemic before it can trigger. This project aligns with the FY23 PRMRP Portfolio Category: Infectious Diseases, FY23 PRMRP Topic: proteomics, and FY23 PRMRP Strategic Goal: Epidemiology: Identify strategies for surveillance or develop modeling tools and/or biomarkers to predict outbreaks or epidemics.
...
%The BioNORAD project aims to develop a groundbreaking platform to proactively identify  the risks posed by emerging Influenza A strains with high pandemic potential, akin to the strategic function of NORAD in defending against geospatial threats.  Influenza A, with widespread prevalence in animal hosts, often re-emerge as novel human pathogens, causing multiple pandemics in the last century. 
% At least four times in the past century, \infl strains spilling over into humans from animal reservoirs have triggered pandemics.
%One approach to mitigating such risks is to identify animal strains that do not yet circulate in humans but are likely to spill over and  achieve human-to-human transmission capability. Although global surveillance efforts collect wild specimens from diverse hosts and geo-locations annually, our ability to reliably and scalably risk-rank individual strains remains limited. The BioNORAD platform, powered by novel pattern discovery algorithms,  offers a  scalable solution to emergence prediction, serving a similar function to the strategic goal of NORAD in the context of defending against geospatial pandemic threats.

\clearpage



% The BioNORAD platform aims to provide a proactive and actionable global surveillance system for emerging pandemic threats from Influenza A, offering significant benefits to military health and the Department of Defense. By enabling early identification and mitigation of emerging pandemic threats, the platform will protect military personnel and assets, enhance military medical response capabilities, strengthen global health security, reduce economic and operational impacts, and promote interdisciplinary collaboration and innovation.
...
% Our model comprises an interdependent collection of local predictors, each aiming to predict the residue at a particular index using residues at other indices as features. These individual predictors are implemented as conditional inference trees which can be trained with sparse noisy data, and resist over-training. The set of residues acting as features in each predictor is automatically identified, revealing complex relationships that capture the intricate rules guiding evolutionary change.



This project aligns with the FY23 PRMRP Portfolio Category: Infectious Diseases, FY23 PRMRP Topic: proteomics, and FY23 PRMRP Strategic Goal: Epidemiology: Identify strategies for surveillance or develop modeling tools and/or biomarkers to predict outbreaks or epidemics.

% The BioNORAD platform offer a novel  approach to preemptively identify and mitigate pandemic risks by leveraging sophisticated machine learning techniques and rigorous data-driven modeling. By bringing together large-scale sequence data from global surveillance efforts and validating predicted variants of viral proteins through in vivo laboratory experiments, we aim to deliver an actionable and deployable platform that optimally exploits the current biosurveillance capacity, identifying when and where an imminent emergence event is likely and if such novel strains are likely to achieve human-to-human transmission capability.

% In conclusion, the BioNORAD platform has the potential to revolutionize pandemic preparedness and response, providing a powerful tool for predicting and mitigating the emergence of novel strains of Influenza A. The platform's ability to automatically factor in the evolving host immunity and the current background environment sets it apart from current state-of-the-art methods, making it a vital asset in the fight against pandemic threats and safeguarding military personnel, assets, and global health security.
...
% BioNORAD: Fast Scalable Pandemic Risk Assessment of Influenza Strains Circulating in Non-human Hosts
...
Subtask 2.1.2 Develop analytics to chart multi-step probabilistic trajectories from observed strains & 12-15 & \checkmark& \\\hline
 ...
 \rowcolor{lightgray} Major Task 2: Assessment of fitness potential emerging zoonotic IAV variants in vitro cell culture& & & \\\hline
