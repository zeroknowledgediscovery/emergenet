...
% \section*{Summary}
% {\bf \itshape Infectious Diseases: (Topic: proteomics) Strategic Goals: Epidemiology: Identify strategies for surveillance or develop modeling tools and/or biomarkers to predict
...
Aim 1: Develop a platform powered by novel pattern discovery and recognition algorithms to automatically parse out emergent evolutionary constraints operating on \infl viruses in the wild, to provide a less-heuristic theory-backed scalable solution to emergence prediction. % Our approach is centred around numerically estimating the probability $Pr(x \rightarrow y)$ of a strain $x$ spontaneously giving rise to  $y$.
