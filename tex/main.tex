...
which reflects evaluations% which  comprises  ten weighted risk elements evaluated 
...
%\DQS{How many qnets do we construct?}
...
% note that using a different subtype-specific enet essetially changes teh metric of comparison, adapating it to the specific subtype.
...
we show that recomputing the risks using \enet{s} constructed from the recent circulating strains instead of using those from when the IRAT assessments took place at the  CDC,  worsens the correlation ($-0.59$, p-value $0.003$).
