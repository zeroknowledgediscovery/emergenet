...
\begin{figure*}[t]
  \tikzexternalenable
  \tikzsetnextfilename{schemetree}
  %\centering

  \begin{tikzpicture}[font=\bf\sffamily\fontsize{8}{8}\selectfont]
 \clip (4in,-0.35in) rectangle (-3.05in,-6.8in);
  ...
  \tikzset{xcirc/.style={circle,inner sep=-25pt,dashed,fill=\ECOLB,opacity=\OPC,rounded corners=5pt,draw=\DCOL,line width=\LWD,scale=\SCALE}}
    ...
    \node[anchor=north,xcirc,label={[yshift=.25in,\LCOL]-90:index 1064}] (P1064) at ([xshift=0.2in,yshift=-0.1in]P1314.south) {\includegraphics[width=\WDT]{Figures/trees/procP1064}};
...
\draw [line width=4pt,\DCOLx,-latex,] (X4)  to [out=-180,in=80,looseness=1]  (P1263);
  ...
  \node[anchor=north,align=left,font=\bf\tt\footnotesize] (N3) at ([yshift=-5.8in,xshift=-1.85in]Z.south) {\bf\sffamily\fontsize{7}{8}\selectfont H1N1 2018\\\bf\sffamily\fontsize{7}{8}\selectfont Haemagglutinin Sequences\\GGAAAACAAAAGCAACAAAA$\cdots$TGA{\large\color{Red1} A}AAAAGA$\cdots$\\AGCAAAAGCAGGGGAAAACA$\cdots $GTT{\large\color{Red1}C}AACCAC$\cdots$\\AGCAAAAGCAGGGGAAAACA$\cdots$GTT{\large\color{Red1}T}AACCAC$\cdots$\\ATGAAGACTATCATTGCTTT$\cdots$ACC{\large\color{Red1}T}TGAGAA$\cdots$};
...
\draw [-{Latex[length=4mm]},ultra thick,line width=\LWD,\ACOL] ([xshift=0.55in,yshift=-.25in]N3.north) --++(0,.4in) node [yshift=-.875in,below,align=center,text=IndianRed2] {residue\\at index 1274};

\node[anchor=east,rounded corners=3pt,align=center] (I1) at ([yshift=.9in,xshift=.22in]P1274.west) {Color\\key$^\bigstar$};

\node[font=\bf\sffamily,anchor=north,rounded corners=3pt,text width=.1in,text height=.1in,fill=Purple1,align=center] (I1) at ([yshift=-.05in]I1.south) {T};

\node[font=\bf\sffamily,anchor=north,rounded corners=3pt,text width=.1in,text height=.1in,fill=DarkOrange3!70,align=center] (I1) at ([yshift=-.05in]I1.south) {A};

\node[font=\bf\sffamily,anchor=north,rounded corners=3pt,text width=.1in,text height=.1in,fill=SeaGreen2,align=center] (I1) at ([yshift=-.05in]I1.south) {C};

\node[font=\bf\sffamily,anchor=north,rounded corners=3pt,text width=.1in,text height=.1in,fill=DodgerBlue2!80,align=center] (I1) at ([yshift=-.05in]I1.south) {G};


\node[anchor=north west] at ([xshift=1in]Z.east) {
\includegraphics[width=3in]{Figures/emergenetscheme}};
...
%
 \else
  \includegraphics[width=\textwidth]{Figures//emergenetscheme}
 \fi
