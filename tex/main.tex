...
\maketitle
  ...
  {\bf \sffamily \fontsize{10}{12}\selectfont \noindent   
    {\normalfont \itshape Abstract:} % Influenza viruses can produce  small sequence alterations~\cite{dos2016influenza}  that sufficiently perturb surface protein structures to evade host immunity.
    % Failing to closely predict the dominant strain in an upcoming season  dramatically reduces  the  effectiveness of the flu shot~\cite{tricco2013comparing}, yet such prediction still remains imperfect.
    %Furthermore,
...
%   \def\LCOL{black!50}
%    \tikzexternalenable
%   \tikzsetnextfilename{tmp}

%  \begin{tikzpicture}[anchor=center,font=\bf\sffamily\fontsize{8}{8}\selectfont]
% \def\DCOL{MediumBlue!40!black}
%   \def\DCOLx{MediumBlue!40!black}
%  %\clip (.75in,-0.27in) rectangle (-2.45in,-7.15in);
...
% \node[anchor=north west,xcirc,inner sep=-35pt,label={[yshift=.9in,xshift=-.5in,align=center,\LCOL]-90:index\\63}] (P63) at ([xshift=-2.7in,yshift=-.75in]Z) {\includegraphics[width=\WDTB]{../qnet_predictions/qnet_models/trees/proc63}}; 
...
%     \node[anchor=north,xcirc,inner sep=-28pt,label={[yshift=.6in,xshift=-.5in,align=center,\LCOL]-90:index\\223}] (P223) at ([xshift=-0in,yshift=-0.10in]P155.south) {\includegraphics[width=\WDT]{../qnet_predictions/qnet_models/trees/proc223}};


%     \node[circle,inner sep=11pt,opacity=.25,draw=black] (X1) at ([yshift=.79in,xshift=.12in]P63) {};
%  \draw [line width=4pt,\DCOLx,-latex,] (X1)  to [out=75,in=140,looseness=1.55]  (P155);

% \node[circle,inner sep=11pt,opacity=.5,draw=black] (X2) at ([yshift=.8in,xshift=-.335in]P155) {};
%  \draw [line width=4pt,\DCOLx,-latex,] (X2)  to [out=0,in=55,looseness=1.2]  (P223);

%  \node[circle,inner sep=11pt,opacity=.5,draw=black] (X3) at ([yshift=.01in,xshift=.1in]P14) {};
%  \draw [line width=4pt,\DCOLx,-latex,] (X3)  to [out=155,in=-130,looseness=1.1]  (P63);

%  \node[circle,inner sep=11pt,opacity=.5,draw=black] (X4) at ([yshift=.83in,xshift=-.35in]P223) {};
% \draw [line width=4pt,\DCOLx,-latex,] (X4)  to [out=210,in=35,looseness=1.2]  (P14);
