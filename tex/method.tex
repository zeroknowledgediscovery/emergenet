...
\Rightarrow  &\sum_{x\in H^t}  \theta(x,\dsta) - \abs{H^t}A \ln \mem{\dsta} \geqq  A \ln \frac{1}{\prod_{x \in H^t} Pr(x \rightarrow \dsta)} \intertext{where $A =\frac{1-\alpha}{\sqrt{8}N^2} $. Since minimizing the LHS maaximizes the lower bound on the probability of the observed strains simulatenously giving rise to $\dsta$, a dominant strain  $\dst$ may be estimated via the optimization:}
&\dst = \argmin_{y \in \cup_{\tau \leqq t} H^\tau} \sum_{x\in H^t}  \theta(x,y) - \abs{H^t}A \ln \mem{y}
\intertext{Further noting that the second term on the right is at least an order of magnitude smaller compared to teh first terem, a good approximation for the optimization may be stated as:}
